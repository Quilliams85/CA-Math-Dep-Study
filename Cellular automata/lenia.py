...
class Automaton():
    def __init__(self):
        ...
        self.last_pot = np.zeros((self.size, self.size))
    ...
    def calculateKernel(self, size, radius, mu=0.5, sigma=0.3):
    ...
    def growth(self, U):
        current = np.sum(U)
        total = U.size
        return 0.1*self.bell(U, 0.15, 0.015)/(current/total) -1
    ...
    def updateCell(self, X):
        ...
        self.age = self.calcAge(X)
        self.last_pot = X
        X = np.clip(X + self.dt * self.growth(U) + 0.1*(self.age), 0, 1)
        return X

    def calcAge(self, X):
        return 10*(X- self.last_pot)
